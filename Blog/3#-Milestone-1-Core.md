...
## 2. One player with a simple sprite without animation.
Done very quickly, once the assets where present.

## 3. Player can move around the level.
...
## 4. Player can place a weight + Weight slams after 4 seconds.
Now the fun begins.
```csharp
...
![Dumbbell pulsation animation clip](animation-clip1.png)
...
## 5. Slam breaks the destructible tiles.
```csharp
...
## 7. Collecting both creatine and whey leads to a win.
